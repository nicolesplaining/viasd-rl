...
\note{
We're Nicole and Harsh, and we're presenting S-four-D, a new state-of-the-art method for hierarchical
...
the gate teaches itself the policy. That is the ``Self-Taught'' in S\textsuperscript{4}D, and it gives us our three contributions.
}
...
accuracy-preserving warm-start. And on calls per token, the exact hardware-independent metric, the
learned gates clearly win.
}
...
the break-even point. The learned gate is the fastest of all the routing methods.
}
...
hand-tuned baseline everywhere on this plot, which makes S\textsuperscript{4}D the state of the art in hierarchical
speculative decoding.
}
...
baseline, free to deploy, and getting better as models improve. That is what makes S\textsuperscript{4}D the state of
