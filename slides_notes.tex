...
\end{frame}
\note{
Hit the numbers hard. One: we match lossless accuracy --- 0.91 --- at four times fewer full-model
calls than greedy, a 2.2x speedup, with zero accuracy lost. Two: it's a tunable frontier --- 2.7x
faster for a tenth of a point of accuracy, or 3.5x for about 0.19, up to ten times fewer expensive
calls. Three: we Pareto-dominate the paper's VIA-SD --- roughly twice its speedup AND 0.3 to 0.5
higher accuracy; where its fixed thresholds collapse to 0.40, we hold 0.71 to 0.91. Four: it scales
with the drafter --- a 3B drafter gives 36x fewer full-model calls at 0.80 accuracy --- and it's a
free-to-deploy tiny MLP that never touches the big model to decide. Tagline: stop hand-picking
thresholds; let the model schedule its own compute.
}
